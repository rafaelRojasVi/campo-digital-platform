...
- **FACT** (2026-09-01) — the full flow (dev login → role-gated upload → async processing → generated artifact → audit trail) is demonstrated end-to-end through a new `/ingesta` page in the existing portal, manually verified in a real browser for ADMIN, OPERATOR, and VIEWER identities, including product isolation;
