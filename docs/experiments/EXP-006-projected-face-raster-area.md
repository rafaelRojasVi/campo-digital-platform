...
17 synthetic NumPy-only tests in `tests/test_projected_face_raster.py` cover: an exact rectangular wall; a sloped bottom boundary; an irregular step top boundary; transverse/depth-protrusion invariance; sparse isolated outliers; internal holes; a second, non-noise but smaller disconnected component; deterministic output; and 8 invalid-configuration cases plus malformed input shape. All pass.
