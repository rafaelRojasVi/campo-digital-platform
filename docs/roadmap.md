...
Per [`DOCUMENTATION_POLICY.md`](DOCUMENTATION_POLICY.md), every phase below
...
- the multi-channel raster export from the existing projected-face pipeline
  ...
  `src/lidar_volume/projected_face_raster.py`; depth channels already exist
  in `src/lidar_volume/front_depth.py` — both would be reused as the shared
