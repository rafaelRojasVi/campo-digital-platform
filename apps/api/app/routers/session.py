...
only, ``app.routers.google_auth`` everywhere), but once a session exists,
...
``app.routers.dev_auth``: mounted only in development, they left no way to
inspect or end a real session anywhere else, even though the portal
(``apps/portal/src/lib/platformApi.ts``) calls both paths unconditionally.
"""
