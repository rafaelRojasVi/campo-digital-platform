...
defect this fixes: the portal (apps/portal/src/lib/platformApi.ts) calls
both paths unconditionally, and a real (Google) session has no other way to
be inspected or ended outside development.
...
def _real_session(client: TestClient) -> str:
    """Establish a real (Google-shaped, non-dev-auth) session; return its raw secret."""
    ...
    with engine.connect() as connection:
        user = resolve_or_create_app_user(
            ...
            identity_key="104728391027364518293",
            display_name="Usuario Campo",
            email="usuario@campodigital.cl",
        )
        grant_product_role(
...
def test_me_returns_identity_and_grants_for_a_real_platform_session(client: TestClient) -> None:
    ...
    body = response.json()
    assert body["display_name"] == "Usuario Campo"
