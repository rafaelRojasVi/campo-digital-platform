...
(`products/lidar/src/lidar_volume/base.py`), an abstract base class that every volume
...
(`products/lidar/src/lidar_core/face_area_reference.py`,
...
1. **Common projected evidence.** Every estimator consumes the same
   ...
   `products/lidar/src/lidar_volume/projected_face_raster.py` and
   `products/lidar/src/lidar_volume/front_depth.py`. No estimator recomputes localization,
...
single simple polygon. `products/lidar/src/lidar_volume/face_boundary.PolygonalMeasurement`
...
- `products/lidar/src/lidar_volume/base.py` — the existing `VolumeEstimator` precedent this
  ADR extends.
- `products/lidar/src/lidar_core/face_area_reference.py` — the existing gated
