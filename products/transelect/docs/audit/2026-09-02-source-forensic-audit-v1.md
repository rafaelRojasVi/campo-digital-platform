...
- CSV export is a hardcoded 17-of-30-field subset, including raw `Estado` but not `Predio Ref`.
...
- **CSV export field set changed**: Actualizable's 17-of-30 export includes `Predio Ref` (new) but drops raw `Estado` (present in v0's export) — a genuine, unexplained difference in "what's useful to export," not an improvement or regression on its face; needs a canonical answer.
